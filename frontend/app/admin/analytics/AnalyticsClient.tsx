...
export default function AnalyticsClient() {
  ...
  const enrollmentsByPlan = facts.reduce(
    (acc, f) => {
  ...
  const endorsementsByType = facts.reduce(
    (acc, f) => {
  ...
  const slaCompliant = events.filter(
    (e) => e.event === "enrollment_workflow_completed",
  ).length;
  const slaFailed = events.filter(
    (e) => e.event === "enrollment_workflow_failed",
  ).length;
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 32 }}>
          <div style={{ flex: 1, minWidth: 350 }}>
            ...
            <ResponsiveContainer width="100%" height={250}>
              <BarChart
                data={Object.entries(enrollmentsByPlan).map(
          ...
          <div style={{ flex: 1, minWidth: 350 }}>
            ...
            <ResponsiveContainer width="100%" height={250}>
              <BarChart
                data={Object.entries(endorsementsByType).map(
                  ([plan, count]) => ({ plan, count }),
                )}
              >
                <XAxis dataKey="plan" />
          ...
          <div style={{ flex: 1, minWidth: 350 }}>
            ...
            <div style={{ fontWeight: 600, marginTop: 8 }}>
              Compliant: {slaCompliant} | Failed: {slaFailed}
            </div>
          </div>
        </div>
      )}
