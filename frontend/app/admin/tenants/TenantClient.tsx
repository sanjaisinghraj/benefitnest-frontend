...
export default function TenantClient() {
  ...
  return (
    <div style={{ padding: 32, maxWidth: 1100, margin: "0 auto" }}>
        ...
        <tbody>
          {tenants.map((t) => (
            <tr
              key={t.id}
              style={{
                background: selectedTenant?.id === t.id ? "#f0f8ff" : undefined,
              }}
            >
              <td>{t.name}</td>
      ...
      {selectedTenant !== null && (
        ...
        >
          ...
          <label>
            Compliance Flags:{" "}
            <input
              value={(form.compliance_flags || []).join(", ")}
              onChange={(e) =>
                handleChange(
                  "compliance_flags",
                  e.target.value.split(",").map((f: string) => f.trim()),
                )
              }
              placeholder="GDPR, HIPAA, IRDAI"
            />
          </label>
          <br />
          <label>
          ...
          <label>
            ...
            <textarea
              ...
              onChange={(e) =>
                handleChange("config_json", JSON.parse(e.target.value))
              }
              rows={6}
              style={{ width: 400 }}
            />
      ...
      {message && (
        <div style={{ marginTop: 24, color: "green" }}>{message}</div>
      )}
    </div>
  );
}
