...
const DonutChart = ({ data, title, onInsightClick, insight, loadingInsight }: { 
  data: ChartData; 
  title: string; 
  onInsightClick: () => void;
  insight?: string;
  ...
  return (
    <div style={{ 
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <h3 style={{ fontSize: '16px', fontWeight: '700', color: colors.gray[800], margin: 0 }}>{title}</h3>
        <button
          onClick={onInsightClick}
          disabled={loadingInsight}
          style={{
            padding: '6px 12px',
            background: loadingInsight ? colors.gray[200] : `linear-gradient(135deg, ${colors.primary}, ${colors.secondary})`,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px', flex: 1 }}>
        <svg viewBox="0 0 200 200" style={{ width: '140px', height: '140px', flexShrink: 0 }}>
          ...
          <text x={center} y={center - 5} textAnchor="middle" style={{ fontSize: '20px', fontWeight: '700', fill: colors.gray[800] }}>
            {total.toLocaleString()}
          </text>
          <text x={center} y={center + 15} textAnchor="middle" style={{ fontSize: '11px', fill: colors.gray[500] }}>
            Total
          </text>
        </svg>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {segments.slice(0, 5).map((seg, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px' }}>
              ...
              <span style={{ color: colors.gray[700], flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{seg.label}</span>
              <span style={{ fontWeight: '600', color: colors.gray[800] }}>{seg.percentage}%</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {insight && (
        <div style={{ 
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
            ...
            <p style={{ fontSize: '12px', color: colors.gray[700], margin: 0, lineHeight: 1.5 }}>{insight}</p>
          </div>
        </div>
      )}
    </div>
...
// Bar Chart Component
const BarChart = ({ data, title, horizontal = false, onInsightClick, insight, loadingInsight }: { 
  data: ChartData; 
  title: string;
  horizontal?: boolean;
  onInsightClick: () => void;
  ...
  return (
    <div style={{ 
      ...
      {horizontal ? (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {data.labels.slice(0, 6).map((label, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <span style={{ fontSize: '11px', color: colors.gray[600], width: '100px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{label}</span>
              <div style={{ flex: 1, height: '24px', backgroundColor: colors.gray[100], borderRadius: '6px', overflow: 'hidden', position: 'relative' }}>
                <div style={{ 
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'flex-end', gap: '8px', paddingTop: '10px' }}>
          {data.labels.map((label, i) => (
            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
              <span style={{ fontSize: '10px', fontWeight: '600', color: colors.gray[700] }}>{data.values[i].toLocaleString()}</span>
              <div style={{ 
                width: '100%', 
              ...
              <span style={{ fontSize: '9px', color: colors.gray[500], textAlign: 'center', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100%' }}>{label}</span>
            </div>
          ))}
        </div>
      )}
...
const AreaChart = ({ data, title, onInsightClick, insight, loadingInsight }: { 
  data: ChartData; 
  title: string;
  onInsightClick: () => void;
  insight?: string;
  ...
  return (
    <div style={{ 
      ...
      <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: 'auto', minHeight: '180px' }}>
        ...
        {points.map((p, i) => (
          <g key={i}>
            <circle cx={p.x} cy={p.y} r="5" fill="white" stroke={colors.primary} strokeWidth="2" />
            <text x={p.x} y={height - 8} textAnchor="middle" style={{ fontSize: '8px', fill: colors.gray[500] }}>
              {data.labels[i]}
            </text>
          </g>
        ))}
    ...
    <div ref={dropdownRef} style={{ position: 'relative', width: '100%', maxWidth: '500px' }}>
      ...
      >
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', flex: 1 }}>
          {selected.length === 0 ? (
            <span style={{ color: colors.gray[400], fontSize: '14px' }}>{placeholder}</span>
          ...
          {selected.length > 3 && (
            <span style={{ 
              ...
              borderRadius: '6px', 
              fontSize: '12px' 
            }}>+{selected.length - 3} more</span>
          )}
        </div>
        <span style={{ color: colors.gray[400], transform: isOpen ? 'rotate(180deg)' : 'none', transition: '0.2s' }}>▼</span>
      </div>
      ...
      {isOpen && (
        <div style={{
          ...
          {/* Quick Actions */}
          <div style={{ padding: '8px 12px', display: 'flex', gap: '8px', borderBottom: `1px solid ${colors.gray[100]}` }}>
            <button
              onClick={selectAll}
              style={{
                padding: '6px 12px',
          ...
          <div style={{ maxHeight: '220px', overflowY: 'auto' }}>
            {filtered.length === 0 ? (
              <div style={{ padding: '20px', textAlign: 'center', color: colors.gray[500], fontSize: '14px' }}>
                No corporates found
              </div>
            ) : (
              filtered.map(corp => (
                <div
                  ...
                  style={{
                    ...
                    gap: '12px',
                    cursor: 'pointer',
                    backgroundColor: selected.includes(corp.tenant_id) ? colors.primaryLight : 'white',
                ...
                >
                  <div style={{
                    ...
                    backgroundColor: selected.includes(corp.tenant_id) ? colors.primary : 'white',
                    display: 'flex',
                    alignItems: 'center',
                  ...
                  <div>
                    <div style={{ fontSize: '14px', fontWeight: '500', color: colors.gray[800] }}>{corp.corporate_legal_name}</div>
                    <div style={{ fontSize: '11px', color: colors.gray[500] }}>{corp.subdomain}.benefitnest.space</div>
                  </div>
                </div>
              ))
            )}
...
export default function ReportsAnalyticsPage() {
  ...
  const getChartInsight = async (chartType: string, chartData: ChartData, chartTitle: string) => {
  ...
  const getOverallInsight = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/admin/claims-analytics/overall-insight`, {
        method: 'POST',
        headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          analytics,
          tenantIds: selectedCorporates,
          corporateNames: selectedCorporates.map(id => corporates.find(c => c.tenant_id === id)?.corporate_legal_name).filter(Boolean)
        })
      });
      const data = await res.json();
      if (data.success) {
    ...
    <div style={{ minHeight: '100vh', backgroundColor: colors.gray[50] }}>
      ...
      <header style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            <h1 style={{ color: 'white', fontSize: '20px', fontWeight: '700', margin: 0 }}>Reports & Analytics</h1>
            <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '12px', margin: 0 }}>Claims Intelligence Dashboard</p>
          </div>
        </div>
          ...
          <div style={{ position: 'relative' }}>
            ...
            {showProfileMenu && (
              <div style={{
                ...
                <div style={{ padding: '16px', borderBottom: `1px solid ${colors.gray[100]}` }}>
                  <div style={{ fontWeight: '600', color: colors.gray[800] }}>{adminProfile?.name}</div>
                  <div style={{ fontSize: '12px', color: colors.gray[500] }}>{adminProfile?.email}</div>
                  <div style={{ marginTop: '6px', display: 'inline-block', padding: '2px 8px', backgroundColor: colors.primaryLight, color: colors.primary, borderRadius: '4px', fontSize: '10px', fontWeight: '600' }}>{adminProfile?.role}</div>
                </div>
                <button
      ...
      <main style={{ padding: '32px', maxWidth: '1600px', margin: '0 auto' }}>
        ...
        <div style={{ 
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ fontSize: '24px' }}>🏢</span>
              <span style={{ fontWeight: '600', color: colors.gray[700] }}>Select Corporates:</span>
            </div>
            <MultiSelectDropdown
              options={corporates}
        ...
        {selectedCorporates.length === 0 && !loading && (
          <div style={{ 
            ...
            <h2 style={{ fontSize: '24px', fontWeight: '700', color: colors.gray[800], marginBottom: '8px' }}>Claims Analysis Dashboard</h2>
            <p style={{ color: colors.gray[500], fontSize: '16px', maxWidth: '500px', margin: '0 auto' }}>
              Select one or more corporates above to view comprehensive claims analytics with AI-powered insights
            </p>
          </div>
        )}
        ...
        {loading && (
          <div style={{ 
            ...
            <h3 style={{ color: colors.gray[600], fontWeight: '500' }}>Analyzing claims data...</h3>
          </div>
        )}
        ...
        {analytics && !loading && (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
              {[
                { label: 'Total Claims', value: analytics.totalClaims.toLocaleString(), icon: '📋', color: colors.primary },
                { label: 'Claimed Amount', value: `₹${(analytics.totalClaimedAmount / 100000).toFixed(1)}L`, icon: '💰', color: colors.warning },
                { label: 'Approved Amount', value: `₹${(analytics.totalApprovedAmount / 100000).toFixed(1)}L`, icon: '✅', color: colors.success },
                { label: 'Rejected Claims', value: analytics.totalRejected.toLocaleString(), icon: '❌', color: colors.danger },
                { label: 'Avg Settlement Days', value: `${analytics.avgSettlementDays} days`, icon: '⏱️', color: colors.info }
              ].map((kpi, i) => (
                <div key={i} style={{ 
                  backgroundColor: 'white', 
                  padding: '20px', 
                  ...
                  <div>
                    <div style={{ fontSize: '12px', color: colors.gray[500], marginBottom: '2px' }}>{kpi.label}</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '24px', marginBottom: '24px' }}>
              ...
              <DonutChart
                ...
                onInsightClick={() => getChartInsight('status', analytics.claimsByStatus, 'Claims by Status')}
              ...
              <DonutChart
                ...
                onInsightClick={() => getChartInsight('type', analytics.claimsByType, 'Claims by Type')}
              ...
              <div style={{ gridColumn: 'span 2' }}>
                <AreaChart
                  ...
                  onInsightClick={() => getChartInsight('monthly', analytics.monthlyTrends, 'Monthly Claims Trend')}
              ...
              <BarChart
                ...
                horizontal={true}
                onInsightClick={() => getChartInsight('hospitals', analytics.topHospitals, 'Top Hospitals by Claims')}
              ...
              <BarChart
                ...
                horizontal={false}
                onInsightClick={() => getChartInsight('category', analytics.claimsByCategory, 'Claims by Category')}
            ...
            <div style={{ 
              ...
              <div style={{ 
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div>
                    <h3 style={{ color: 'white', fontSize: '18px', fontWeight: '700', margin: 0 }}>AI-Powered Claims Intelligence</h3>
                    <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '12px', margin: 0 }}>Comprehensive analysis and predictive insights</p>
                  </div>
                </div>
                <button
                  ...
                  style={{
                    padding: '10px 20px',
                    backgroundColor: loadingOverallInsight ? 'rgba(255,255,255,0.3)' : 'white',
                    color: loadingOverallInsight ? 'rgba(255,255,255,0.7)' : colors.primary,
                    border: 'none',
                    borderRadius: '10px',
                    fontSize: '14px',
                    fontWeight: '600',
                ...
                >
                  {loadingOverallInsight ? '⏳ Generating...' : '✨ Generate Full Analysis'}
                </button>
              </div>
              ...
              <div style={{ padding: '24px', minHeight: '200px' }}>
                {!overallInsight && !loadingOverallInsight && (
                  <div style={{ textAlign: 'center', padding: '40px' }}>
                    ...
                    <p style={{ color: colors.gray[500], fontSize: '15px' }}>
                      Click &quot;Generate Full Analysis&quot; to get AI-powered insights on your claims data,
                      including risk predictions, cost optimization opportunities, and strategic recommendations.
                    </p>
                  </div>
                )}
                ...
                {loadingOverallInsight && (
                  <div style={{ textAlign: 'center', padding: '40px' }}>
                    ...
                    <p style={{ color: colors.gray[600], fontWeight: '500' }}>AI is analyzing your claims data...</p>
                    <p style={{ color: colors.gray[400], fontSize: '13px' }}>This may take a few moments</p>
                  </div>
                )}
                ...
                {overallInsight && !loadingOverallInsight && (
                  <div style={{ 
                    ...
                    {overallInsight.split('\n').map((paragraph, i) => (
                      <p key={i} style={{ 
                        marginBottom: '16px',
                        padding: paragraph.startsWith('•') || paragraph.startsWith('-') ? '0 0 0 16px' : '0'
                      }}>
                        {paragraph.startsWith('**') && paragraph.endsWith('**') ? (
                          <strong style={{ color: colors.gray[800], fontSize: '16px' }}>
                            {paragraph.replace(/\*\*/g, '')}
                          </strong>
                        ) : paragraph.startsWith('##') ? (
                          <strong style={{ color: colors.primary, fontSize: '17px', display: 'block', marginTop: '12px' }}>
