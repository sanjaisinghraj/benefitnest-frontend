...
export default function TenantAnalyticsPage() {
  ...
  const [branding, setBranding] = useState<any>({ font: "Inter, sans-serif", background: "#fff", color: "#222" });
  ...
  return (
    <div style={{ fontFamily: branding.font, background: branding.background, color: branding.color, minHeight: '100vh', padding: 32 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <button onClick={() => {}} style={{ marginLeft: 8 }}>Export XLSX</button>
        <button onClick={() => {}} style={{ marginLeft: 8 }}>Export PDF</button>
      </div>
      {(isSuperAdmin || isCorporateAdmin) && (
        <div style={{ marginBottom: 16, padding: 8, border: '1px solid #eee', background: '#fafafa' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 8 }}>
              ...
              <tbody>
                {Object.entries(complianceByCountry).map(([country, data]) => {
                  const d = data as { GDPR: number; HIPAA: number; IRDAI: number; total: number };
                  return (
                    <tr key={country}>
                      <td>{country}</td>
                      <td>{d.GDPR}</td>
