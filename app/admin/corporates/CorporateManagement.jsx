...
const CorporateManagement = () => {
  ...
  const fetchDynamicFields = async () => {
    try {
      const response = await axios.get(`${MASTERS_API}/tenants/structure`, {
        headers: getAuthHeaders(),
      });
      if (response.data.success && response.data.columns) {
        const allColumns = response.data.columns.map((c) => c.column_name);
        const newFields = allColumns.filter(
          (col) => !CORE_FIELDS.includes(col),
        );
  ...
  const fetchCorporates = async (page = 1) => {
    try {
      ...
      const response = await axios.get(CORPORATES_API, {
        headers: getAuthHeaders(),
        params,
      });
      if (response.data.success) {
  ...
  const fetchStats = async () => {
    try {
      const [allRes, activeRes, pipelineRes, expiringRes] = await Promise.all([
        ...
        axios.get(CORPORATES_API, {
          headers: getAuthHeaders(),
          params: { limit: 1, status: "INACTIVE" },
        }),
        axios.get(CORPORATES_API, {
          headers: getAuthHeaders(),
          params: { limit: 1, contract_expiring_days: 90 },
        }),
      ]);
      setStats({
  ...
  const runAIValidation = async () => {
    ...
    try {
      ...
      const existingCorp = corporates.find(
        (c) =>
      ...
      if (!selectedCorporate) {
        const subdomainExists = corporates.find(
          (c) =>
        ...
        const codeExists = corporates.find(
          (c) =>
            c.tenant_code?.toLowerCase() ===
            formData.tenant_code?.toLowerCase(),
        );
        if (codeExists) {
          issues.push({
            field: "tenant_code",
      ...
      if (formData.country === "India") {
        ...
        if (!hasSuffix && name.length > 0) {
          issues.push({
            ...
            severity: "warning",
            message:
              "Indian company names typically end with Private Limited, LLP, or similar suffix",
      ...
      if (
        ...
        issues.push({
          ...
          severity: "error",
          message:
            "Invalid GSTIN format. Expected: 22AAAAA0000A1Z5 (15 characters)",
        });
      }
      ...
      if (validContacts.length === 0) {
        issues.push({
          ...
          severity: "warning",
          message:
            "No valid contacts added. Consider adding at least one contact with name and email.",
        });
      }
  ...
  const handleEdit = (corporate) => {
    ...
    setFormData({
      ...
      benefitnest_manager: corporate.benefitnest_manager || {
        name: "",
        email: "",
        phone: "",
      },
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      setToast({
        message: err.response?.data?.message || "Failed to delete corporate",
        type: "error",
      });
    }
  };
  ...
  const handleSave = async (skipAI = false) => {
    ...
    if (
      ...
      !formData.subdomain ||
      !formData.corporate_legal_name
    ) {
      setToast({
        message:
          "Please fill all required fields (Code, Subdomain, Legal Name)",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      if (selectedCorporate) {
        ...
        if (response.data.success) {
          ...
          throw new Error(
            response.data.message || "Update failed - server returned false",
          );
        }
      } else {
        ...
        const response = await axios.post(CORPORATES_API, payload, {
          headers: getAuthHeaders(),
        });
        ...
        if (response.data.success) {
          ...
          throw new Error(
            response.data.message || "Create failed - server returned false",
          );
        }
      }
    } catch (err) {
  ...
  const handleAddField = async () => {
    try {
      ...
      const response = await axios.post(
        `${MASTERS_API}/tenants/add-field`,
        newField,
        { headers: getAuthHeaders() },
      );
      if (response.data.success) {
        setToast({
          message: response.data.message || "Field added successfully",
          type: "success",
        });
      ...
      setToast({
        message: err.response?.data?.message || "Failed to add field",
        type: "error",
      });
    }
  };
    ...
    try {
      ...
      try {
        ...
        const response = await axios.get(
          `${CORPORATES_API}/${corporate.tenant_id}/check-portal`,
          { headers: getAuthHeaders() },
        );
        ...
        if (response.data.success && response.data.data.portal_exists) {
          ...
          const portalUrl =
            ...
            `https://${corporate.subdomain}.benefitnest.space`;
  ...
  const handleCreatePortal = async () => {
    ...
    try {
      console.log(
        `[PORTAL] Creating portal for tenant: ${portalCorporate.tenant_id}`,
      );
      const createResponse = await axios.post(
        `${CORPORATES_API}/${portalCorporate.tenant_id}/create-portal`,
        {},
        { headers: getAuthHeaders() },
      );
      ...
      if (createResponse.data.success) {
        ...
        const portalUrl =
          createResponse.data.data?.portal_url ||
          `https://${portalCorporate.subdomain}.benefitnest.space`;
        ...
        throw new Error(
          createResponse.data.message || "Failed to create portal",
        );
      }
    } catch (createErr) {
  ...
  const handleDownloadData = async () => {
    try {
      ...
      if (response.data.success && response.data.data) {
        await downloadExcel(
          response.data.data,
          `corporates_data_${new Date().toISOString().split("T")[0]}.xlsx`,
        );
        setToast({
          message: `Downloaded ${response.data.data.length} records!`,
          type: "success",
        });
      }
    } catch (err) {
  ...
  const handleBulkUpload = async (e) => {
    ...
    try {
      ...
      setToast({
        message: `Uploaded ${success} of ${jsonData.length} corporates${failed > 0 ? ` (${failed} failed)` : ""}`,
  ...
  const renderStatusBadge = (status) => {
    ...
    return (
      <Badge variant={variants[status] || "default"}>{status || "N/A"}</Badge>
    );
  };
  ...
  return (
    ...
    >
      ...
      <main style={{ padding: "24px", maxWidth: "1600px", margin: "0 auto" }}>
        ...
        >
          <div>
            ...
            <p style={{ color: colors.gray[500], marginTop: "4px" }}>
              Manage corporate clients, portals, and contacts
            </p>
          </div>
          <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
            ...
            <Button variant="cyan" icon="📊" onClick={handleDownloadData}>
              Download
            </Button>
            <Button
              variant="warning"
            ...
            <Button variant="outline" icon="🗂️" onClick={fetchSchema}>
              Schema
            </Button>
          </div>
        </div>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                fontWeight: "600",
                color:
                  activeTab === tab.key ? colors.primary : colors.gray[600],
                backgroundColor: "transparent",
                border: "none",
                borderBottom: `2px solid ${activeTab === tab.key ? colors.primary : "transparent"}`,
                cursor: "pointer",
                marginBottom: "-1px",
              }}
            >
        ...
        >
          <div style={{ flex: 1, minWidth: "250px" }}>
            <input
              type="text"
              placeholder="Search by name, code, subdomain, industry..."
              value={searchQuery}
          ...
          <div style={{ minWidth: "200px" }}>
            ...
            >
              ...
              {industryTypes.map((ind) => (
                <option key={ind.id || ind.code || ind.name} value={ind.name}>
                  {ind.name}
                </option>
              ))}
            </select>
                  ...
                  corporates.map((corp) => (
                    ...
                    >
                      <td style={{ padding: "12px 16px" }}>
                        ...
                        >
                          {corp.branding_config?.logo_url ? (
                            <img
                              src={corp.branding_config.logo_url}
                              alt=""
                              style={{
                                width: "100%",
                                height: "100%",
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        ...
                        <div
                          style={{ fontSize: "12px", color: colors.gray[500] }}
                        >
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        {renderStatusBadge(corp.status)}
                      </td>
                      <td style={{ padding: "12px 16px" }}>
                        {renderHealthBadge(corp.health_score)}
                      </td>
                      <td style={{ padding: "12px 16px" }}>
                        {corp.subdomain && (
                          <Button
                            ...
                            variant="outline"
                            icon="🔗"
                            onClick={() => checkAndOpenPortal(corp)}
                          >
                            Open
                          </Button>
                        )}
                      ...
                      <td style={{ padding: "12px 16px", textAlign: "right" }}>
                        ...
                        >
                          ...
                          <Button
                            ...
                            variant="danger"
                            icon="🗑️"
                            onClick={() => handleDeleteClick(corp)}
                          >
                            Delete
                          </Button>
                        </div>
      ...
      <Modal
        ...
        title={selectedCorporate ? "Edit Corporate" : "Add New Corporate"}
        icon={selectedCorporate ? "✏️" : "➕"}
        size="xl"
      >
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          ...
          <div>
            ...
            >
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  setFormData({
                    ...formData,
                    subdomain: val,
                    portal_url: `https://${val}.benefitnest.space`,
                  });
                }}
                placeholder="e.g., acmecorp"
                disabled={!!selectedCorporate}
                hint="Portal URL prefix"
              />
              <Input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    corporate_legal_name: e.target.value,
                  })
                }
              ...
              <Select
                ...
                options={corporateTypes.map((t) => ({
                  value: t.name,
                  label: t.name,
                }))}
                value={formData.corporate_type || ""}
                onChange={(e) =>
                  setFormData({ ...formData, corporate_type: e.target.value })
                }
              />
              <Select
                label="Industry Type"
                options={industryTypes.map((t) => ({
                  value: t.name,
                  label: t.name,
                }))}
          ...
          <div>
            ...
            >
              <Input
                label="PAN"
                value={formData.registration_details?.pan || ""}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    registration_details: {
                      ...formData.registration_details,
              ...
              <Input
                label="GSTIN"
                value={formData.registration_details?.gstin || ""}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    registration_details: {
              ...
              <Input
                label="CIN"
                value={formData.registration_details?.cin || ""}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    registration_details: {
                ...
                hint="21-character CIN"
              />
              <Input
                label="TAN"
                value={formData.registration_details?.tan || ""}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    registration_details: {
          ...
          <div>
            ...
            {contacts.map((contact, index) => (
              <div
                key={index}
                style={{
                  backgroundColor: contact.is_primary
                    ? colors.primaryLight
                    : colors.gray[50],
                  padding: "16px",
                  borderRadius: "12px",
                ...
                >
                  ...
                  >
                    ...
                    {contact.is_decision_maker && (
                      <Badge variant="info">Decision Maker</Badge>
                    )}
                  </div>
                  {contacts.length > 1 && (
                    <Button
          ...
          <div>
            ...
            >
              <Input
                label="Manager Name"
                value={formData.benefitnest_manager?.name || ""}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    benefitnest_manager: {
                      ...formData.benefitnest_manager,
                      name: e.target.value,
                    },
                  })
                }
              ...
              <Input
                label="Manager Email"
                type="email"
                value={formData.benefitnest_manager?.email || ""}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    benefitnest_manager: {
              ...
              <Input
                label="Manager Phone"
                value={formData.benefitnest_manager?.phone || ""}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    benefitnest_manager: {
          ...
          <div>
            ...
            <TextArea
              ...
              placeholder="Any internal notes about this corporate (not visible to client)..."
        ...
        >
          ...
          <div style={{ display: "flex", gap: "12px" }}>
            ...
            <Button
              ...
              loading={savingRecord || aiValidating}
            >
              {aiValidating
                ? "🤖 Validating..."
                : selectedCorporate
        ...
        (aiValidationResults.issues?.length ?? 0) === 0 ? (
          ...
          <>
            ...
            >
              ...
              >
                ⚠️ Found {aiValidationResults.issues?.length || 0} potential
                issue(s)
              </div>
                ...
                >
                  ...
                  >
                    ...
                    <strong style={{ color: colors.gray[800] }}>
                      {issue.field?.replace(/_/g, " ").replace(/\./g, " → ")}
                    </strong>
                  </div>
      ...
      >
        {selectedCorporate && (
          ...
          >
            {Object.entries(selectedCorporate)
      ...
      <Modal
        ...
        title="Bulk Upload Corporates"
        icon="📤"
        size="md"
      >
        <p style={{ color: colors.gray[600], marginBottom: "20px" }}>
          Upload an Excel file with corporate data. Download the template first
        ...
        >
          ...
          >
            <div>
              <div style={{ fontWeight: "600", color: colors.cyan }}>
            ...
            <Button variant="cyan" size="sm" onClick={handleDownloadTemplate}>
              Download
            </Button>
          </div>
        </div>
        ...
        >
          ...
          <div style={{ fontWeight: "600", color: colors.gray[700] }}>
      ...
      >
        <div style={{ textAlign: "center", padding: "20px 0" }}>
          ...
          <p style={{ color: colors.gray[600], marginBottom: "8px" }}>
      ...
      >
        <div style={{ textAlign: "center", padding: "20px 0" }}>
          ...
          >
            The employee portal for{" "}
            <strong>{portalCorporate?.corporate_legal_name}</strong> hasn't been
            set up yet.
          </p>
          <div
            style={{
          ...
          <p style={{ fontSize: "13px", color: colors.gray[500] }}>
            Creating the portal will enable employee self-service features
            including health claims, policies, and family management.
          </p>
        </div>
        <div
          style={{
      ...
      >
                  ...
                  {tableStructure.columns?.map((col) => (
                    ...
                    >
                      ...
                      <td style={{ padding: "10px 12px", fontSize: "14px" }}>
                        <Badge variant="cyan">{col.data_type}</Badge>
                      </td>
                      <td style={{ padding: "10px 12px", fontSize: "14px" }}>
                        {col.is_nullable === "YES" ? "✅" : "❌"}
                      </td>
        ...
        >
          <Button variant="outline" onClick={() => setShowSchemaModal(false)}>
            Close
          </Button>
        </div>
      </Modal>
      ...
      <Modal
        ...
        title="Add New Field to Tenants Table"
        icon="➕"
        size="md"
      >
        <div
        ...
        >
          <div style={{ fontSize: "13px", color: colors.warning }}>
            ⚠️ <strong>Warning:</strong> This will add a new column to the
            database. Make sure you know what you're doing.
          </div>
        </div>
        <Input
          label="Column Name"
        ...
        <Select
          ...
          onChange={(e) =>
            setNewField({ ...newField, data_type: e.target.value })
          }
          options={[
        ...
        <div style={{ marginBottom: "16px" }}>
          <Checkbox
            label="Allow NULL values (empty)"
            checked={newField.is_nullable}
            onChange={(e) =>
              setNewField({ ...newField, is_nullable: e.target.checked })
            }
          />
        </div>
        <Input
        ...
        >
          ...
          <Button variant="warning" onClick={handleAddField}>
            Add Field
          </Button>
        </div>
      </Modal>
