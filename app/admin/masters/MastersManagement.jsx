...
const FIELD_TYPE_MAP = {
  ...
  notes: "textarea",
  corporate_type: "dropdown",
  industry_type: "dropdown",
  insurer_id: "dropdown",
  tpa_id: "dropdown",
  broker_id: "dropdown",
  account_manager_id: "dropdown",
  entity_type: "dropdown",
  office_type: "dropdown",
  hierarchy_type: "dropdown",
  country: "dropdown",
};
...
const MastersManagement = () => {
  ...
  const fetchLookups = async () => {
    try {
      const response = await axios.get(`${MASTERS_API}/lookups/all`, {
        headers: getAuthHeaders(),
      });
      if (response.data.success) setLookups(response.data.lookups || {});
    } catch (err) {
  ...
  const fetchTables = async () => {
    try {
      const response = await axios.get(`${MASTERS_API}/tables`, {
        headers: getAuthHeaders(),
      });
  ...
  const fetchSchema = async (table) => {
    try {
      const response = await axios.get(`${MASTERS_API}/${table}/schema`, {
        headers: getAuthHeaders(),
      });
  ...
  const fetchTableStructure = async (table) => {
    try {
      const response = await axios.get(`${MASTERS_API}/${table}/structure`, {
        headers: getAuthHeaders(),
      });
      ...
      setToast({ message: "Failed to fetch table structure", type: "error" });
    }
  };
  const fetchFKOptions = async (table) => {
    try {
      const response = await axios.get(`${MASTERS_API}/${table}/fk-options`, {
        headers: getAuthHeaders(),
      });
  ...
  const checkDependencies = async (table, id) => {
    try {
      const response = await axios.get(
        `${MASTERS_API}/${table}/${id}/dependencies`,
        { headers: getAuthHeaders() },
      );
  ...
  const primaryKeyColumn = useMemo(() => {
    ...
    const pkCol = columns.find(
      (c) => c.column_name === "id" || c.column_name.endsWith("_id"),
    );
    return pkCol?.column_name || columns[0]?.column_name || "id";
  }, [columns]);
    ...
    () =>
      columns.filter(
        (c) =>
          !["created_at", "updated_at", "created_by", "updated_by"].includes(
            c.column_name,
          ) && c.column_name !== primaryKeyColumn,
      ),
  ...
  const getDropdownOptions = (columnName) => {
    ...
    if (columnName === "corporate_type" && lookups.corporate_types)
      return lookups.corporate_types.map((t) => ({
        value: t.id || t.code,
        label: t.name,
      }));
    if (columnName === "industry_type" && lookups.industry_types)
      return lookups.industry_types.map((t) => ({
        value: t.id || t.code,
        label: t.name,
      }));
    if (columnName === "insurer_id" && lookups.insurers)
      return lookups.insurers.map((t) => ({
        value: t.insurer_id,
        label: t.name,
      }));
    if (columnName === "tpa_id" && lookups.tpas)
      return lookups.tpas.map((t) => ({ value: t.tpa_id, label: t.name }));
    if (columnName === "broker_id" && lookups.brokers)
      return lookups.brokers.map((t) => ({
        value: t.broker_id,
        label: t.name,
      }));
    if (columnName === "account_manager_id" && lookups.account_managers)
      return lookups.account_managers.map((t) => ({
        value: t.manager_id,
        label: t.name,
      }));
    return null;
  ...
  const handleEdit = (record) => {
    ...
    const data = {};
    editableColumns.forEach((col) => {
      data[col.column_name] = record[col.column_name] ?? "";
    });
    setFormData(data);
    setShowForm(true);
  };
  const handleDeleteClick = async (record) => {
    setSelectedRecord(record);
    const depData = await checkDependencies(
      selectedTable,
  ...
  const handleSave = async (skipAI = false, ignoreWarnings = false) => {
    try {
      ...
      const payload = selectedRecord
        ? {
            updates: formData,
            skip_ai_validation: skipAI,
            ai_warnings_ignored: ignoreWarnings,
          }
        : {
            record: formData,
            skip_ai_validation: skipAI,
      ...
      if (response.data.success) {
        setToast({
          message: selectedRecord ? "Record updated!" : "Record created!",
          type: "success",
        });
        setShowForm(false);
  ...
  const handleDownloadData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        `${MASTERS_API}/${selectedTable}/download/all`,
        { headers: getAuthHeaders() },
      );
      if (response.data.success && response.data.data) {
        await downloadExcel(
          response.data.data,
          `${selectedTable}_data_${new Date().toISOString().split("T")[0]}.xlsx`,
        );
        setToast({
  ...
  const handleBulkUpload = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${MASTERS_API}/${selectedTable}/bulk`,
        { records: jsonData, skip_ai_validation: false },
        { headers: getAuthHeaders() },
      );
      if (response.data.requires_confirmation) {
        setValidationWarnings(response.data.validation?.warnings || []);
        ...
        const msg = response.data.results
          ? `Uploaded ${response.data.results.success} of ${jsonData.length} records`
          : response.data.message;
        setToast({
          message: msg,
          type: response.data.results?.failed > 0 ? "warning" : "success",
        });
  ...
  const handleBulkUploadIgnoreWarnings = async () => {
    ...
    try {
      const response = await axios.post(
        `${MASTERS_API}/${selectedTable}/bulk`,
        {
          records: pendingUploadData,
          skip_ai_validation: true,
          ai_warnings_ignored: true,
        },
        { headers: getAuthHeaders() },
      );
      if (response.data.success) {
  ...
  const handlePasswordSubmit = async () => {
    try {
      ...
      // Run AI validation
      const response = await axios.post(
        `${MASTERS_API}/${selectedTable}/validate-all`,
        {},
        { headers: getAuthHeaders() },
      );
      if (response.data.success) {
  ...
  const handleAddField = async () => {
    try {
      ...
      const response = await axios.post(
        `${MASTERS_API}/${selectedTable}/add-field`,
        newField,
        { headers: getAuthHeaders() },
      );
      if (response.data.success) {
        ...
        setToast({ message: "Run SQL manually", type: "warning" });
        alert(`Run this SQL:\n\n${response.data.sql}`);
      }
    } catch (err) {
      setToast({
  ...
  const renderFormField = (col) => {
    ...
    if (dropdownOptions || fieldType === "dropdown")
      return (
        <Select
          key={columnName}
          label={label}
    ...
    if (fieldType === "boolean")
      return (
        <Checkbox
          ...
          label={label}
          checked={
            formData[columnName] === true ||
            formData[columnName] === "true" ||
            formData[columnName] === "t"
          }
          onChange={(e) =>
            setFormData({ ...formData, [columnName]: e.target.checked })
          }
        />
      );
    ...
    if (fieldType === "date")
      return (
        <Input
          ...
          label={label}
          type="date"
          value={formData[columnName] ? formData[columnName].split("T")[0] : ""}
          onChange={(e) =>
            setFormData({ ...formData, [columnName]: e.target.value })
          }
        />
    ...
    if (fieldType === "datetime")
      return (
        <Input
          ...
          label={label}
          type="datetime-local"
          value={formData[columnName] ? formData[columnName].slice(0, 16) : ""}
          onChange={(e) =>
            setFormData({ ...formData, [columnName]: e.target.value })
          }
        />
  ...
  return (
    ...
    >
      ...
      <main style={{ maxWidth: "1600px", margin: "0 auto", padding: "24px" }}>
        ...
        >
          ...
          <StatsCard
            ...
            label={
              selectedTable
                ? `${TABLE_LABELS[selectedTable] || selectedTable} Records`
                : "Select a Table"
            }
            icon="📊"
          />
        ...
        >
          ...
          >
            <div style={{ flex: "1", minWidth: "250px", maxWidth: "400px" }}>
              <Select
                ...
                options={tables.map((t) => ({
                  value: t.name,
                  label: `${TABLE_LABELS[t.name] || t.name} (${t.count})`,
                }))}
            ...
            {selectedTable && (
              <>
                <Button variant="cyan" icon="+" onClick={handleAdd}>
                  Add New
                </Button>
                <Button
                  variant="success"
                ...
                <tbody>
                  {loading ? (
                    ...
                    records.map((record, idx) => (
                      <tr
                        key={record[primaryKeyColumn] || idx}
                        style={{
                        ...
                        {displayColumns.slice(0, 6).map((col) => (
                          ...
                          >
                            {col.data_type === "boolean" ? (
                              <Badge
                                variant={
                                  record[col.column_name] ? "success" : "danger"
                                }
                              >
                                {record[col.column_name] ? "Yes" : "No"}
                              </Badge>
                            ) : (
                              String(record[col.column_name] ?? "—").substring(
                                0,
                                50,
                              )
                            )}
        ...
        {!selectedTable && (
          ...
          >
            ...
            <p style={{ color: colors.gray[500] }}>
              Choose a table from the dropdown above
            </p>
          </div>
        )}
      </main>
          ...
          >
            {columns.map((col) => (
              ...
              >
                ...
                >
                  {col.data_type === "boolean"
                    ? selectedRecord[col.column_name]
                      ? "✅ Yes"
                      : "❌ No"
      ...
      >
        ...
        >
          {editableColumns.map((col) => renderFormField(col))}
        </div>
        <div
          style={{
        ...
        >
          ...
          >
            ...
            <span style={{ fontSize: "13px", fontWeight: "500" }}>
              AI will automatically validate your data before saving
            </span>
          </div>
        </div>
        <div
      ...
      >
        <p style={{ color: colors.gray[600], marginBottom: "20px" }}>
          Upload an Excel file. Download template first.
        </p>
        <div
          style={{
            backgroundColor: colors.primaryLight,
        ...
        >
          ...
          >
            <span>🤖</span>
            <span style={{ fontSize: "13px", fontWeight: "500" }}>
              AI will validate all records before uploading
            </span>
          </div>
        </div>
        <div
          ...
          >
            ...
            <ul style={{ margin: 0, paddingLeft: "20px" }}>
              {dependencies.map((dep, i) => (
                <li key={i}>
                  {dep.count} records in {dep.table}
                </li>
              ))}
            </ul>
          </div>
      ...
      >
            ...
            {validationWarnings.map((w, i) => (
              <div
                ...
                style={{
                  ...
                  marginBottom: "8px",
                  fontSize: "14px",
                  borderLeft: `4px solid ${w.severity === "error" ? colors.danger : colors.warning}`,
                }}
              >
                <div
                  style={{
                ...
                >
                  <Badge
                    variant={w.severity === "error" ? "danger" : "warning"}
                  >
                    {w.severity || "warning"}
                  </Badge>
                  <strong>
                ...
                <div style={{ color: colors.gray[700] }}>{w.message}</div>
                {w.suggested_value && (
                  <div
                    style={{
                      fontSize: "12px",
        ...
        >
          ...
          <Button
            variant="warning"
            onClick={() => {
              if (pendingUploadData) handleBulkUploadIgnoreWarnings();
      ...
      <Modal
        isOpen={showAIValidateModal}
        onClose={() => setShowAIValidateModal(false)}
      ...
      >
        {validationWarnings.length === 0 ? (
          <div style={{ textAlign: "center", padding: "40px" }}>
            ...
            <p style={{ color: colors.gray[500] }}>
              No issues found in the selected records.
            </p>
          </div>
        ) : (
          <>
            ...
            >
              <div style={{ fontWeight: "600", color: colors.warning }}>
                Found {validationWarnings.length} potential issues
              </div>
            </div>
            <div style={{ maxHeight: "400px", overflowY: "auto" }}>
              {validationWarnings.map((w, i) => (
                ...
                >
                  ...
                  <p style={{ margin: "4px 0 0", color: colors.gray[700] }}>
                    {w.message}
                  </p>
                  {w.suggested_value && (
                    <div
        ...
        >
          <Button
            variant="outline"
            onClick={() => setShowAIValidateModal(false)}
          >
            Close
          </Button>
        </div>
      ...
      <Modal
        ...
        title="AI Validation - Authentication Required"
        icon="🔐"
        size="md"
      >
        <div
        ...
        >
          ...
          >
            <li style={{ marginBottom: "6px" }}>
              Validating large tables can{" "}
              <strong>exceed API rate limits</strong>
            </li>
            <li style={{ marginBottom: "6px" }}>
              AI server may <strong>block requests</strong> if overused
            </li>
            <li style={{ marginBottom: "6px" }}>
              For large tables, validation results may be{" "}
              <strong>incomplete or inaccurate</strong>
            </li>
            <li style={{ marginBottom: "6px" }}>
              <strong>Recommended:</strong> Use AI validation on Add/Edit for
              single records
            </li>
          </ul>
        </div>
        ...
        >
          ...
          <Button
            ...
            disabled={!adminPassword || aiValidating}
      ...
      >
        {tableStructure && (
          <>
            ...
            {tableStructure.referenced_by?.length > 0 && (
              <div style={{ marginTop: "24px" }}>
                ...
                {tableStructure.referenced_by.map((ref, i) => (
                  <Badge
                    key={i}
                    variant="info"
